...
namespace GrupoTecnofix_Api.Data.Repositories
{
    public class UsuariosRepository : IUsuariosRepository
    {
        ...
        public async Task<PagedResult<UsuarioListDto>> GetListAsync(int page, int pageSize, string? search, CancellationToken ct)
        {
        ...
        public Task<List<int>> GetPerfisAsync(int idUsuario, CancellationToken ct)
            => _db.UsuariosPerfis
                .AsNoTracking()
        ...
        public async Task ReplacePerfisAsync(int idUsuario, IEnumerable<int> perfis, CancellationToken ct)
        {
            var atuais = _db.UsuariosPerfis.Where(x => x.IdUsuario == idUsuario);
            _db.UsuariosPerfis.RemoveRange(atuais);

            foreach (var pid in perfis.Distinct())
            {
                await _db.UsuariosPerfis.AddAsync(new UsuariosPerfi
                {
                    IdUsuario = idUsuario,
                    IdPerfil = pid
                }, ct);
